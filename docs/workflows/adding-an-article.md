...
2. Draft in `drafts/<slug>.md` first. Sit on it overnight. Cold re-read tomorrow. **Pick the slug with the project's pattern in mind:** the V&V arc uses sentence-form slugs mirroring the title (`verification-is-a-workflow-problem`, `the-model-is-not-the-grader`, `senior-developers-trust-ai-less`, `the-work-is-splitting`); artifact-form / juxtaposition slugs (`plausibility-vs-correctness`) break that pattern. A working argument-form title yields a working sentence-form slug. Get this right at draft time to avoid trailing rename debt across cross-references in sibling drafts.
3. **Build a verification record at `docs/verification/<slug>.md`** for every load-bearing statistic, named study, or coined attribution. Apply Step 0 + Steps 4–6 of the [agent-ready-papers](file:///home/jeroen/repos/agent-ready-papers/templates/anti-hallucination.md) checklist; trace each number to primary source; map confidence tier to article language per `docs/writing-guide.md` Section 7. If a claim cannot be traced to primary source, weaken the language or drop the claim.
...
10. **Cross-model review pass.** Paste the article body (or the live preview URL once deployed) into a different model (GPT, Gemini) or a fresh Claude session with no project context, using `templates/review-prompt.md` **(`templates/review-prompt.md` was not in `~/repos/agent-ready-papers/templates/` as of 2026-08-27 — the templates there are anti-hallucination, claim-registry, cost-log, decision-record, glossary, hypothesis-log, key-quotes, vv-framework, work-item, writing-guide. Use any cross-model review prompt until it reappears.)** Variant B (non-empirical) as the prompt. Address logic gaps, unsupported assertions, citation drift, and tone issues before pushing. Filter the review output against the project's voice rules in `docs/writing-guide.md` Section 6 (cross-vendor reviewers do not read your voice and will frequently suggest label-coining, subheadings, or popular-psychology references that the rule rejects). Per `docs/writing-guide.md` Section 7.
11. Commit and push to `main`. Netlify rebuilds and deploys. **Pre-push gates:**
    - **Verification record present or skip recorded.** Confirm that step 3 was resolved either way. Either `docs/verification/<slug>.md` exists, or the draft's status header records why it was skipped. A missing verification record without a recorded skip-decision is the gotcha-log 2026-06-02 failure mode.
    - **Linked external repos are publicly accessible.** If the article inline-links to a GitHub repo (notably `agent-ready-papers` in the framework-handoff family), confirm public visibility before push: `gh api repos/<owner>/<repo> --jq .visibility` should return `public`. A 404'd primary-source link survives every other check, including the cross-model review pass that operates on prose rather than transport (gotcha-log 2026-06-05). This gate retroactively unbreaks earlier articles too: when `agent-ready-papers` flipped public on 2026-06-05, the same link in `verification-is-a-workflow-problem.astro` resolved without any code change. The pattern: link-visibility lives in repo-visibility, not in the .astro file.
