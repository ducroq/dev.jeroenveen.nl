...
**To revisit in ~5–7 days:**
- Compare reception (Pulse views, short-post reactions, comments) against `the-work-is-splitting` and `senior-developers-trust-ai-less`. Different hashtag set + different cover style + tighter audience filter — three variables moved at once, so reception data is mixed-signal, but worth logging.
- Does the diagram-as-cover variant earn more click-through than the typographic covers? If yes, promote as the default for articles with a strong central figure; if no, keep typographic as default (per `docs/workflows/adding-an-article.md` step 7).
- Watch for "is this vmodel.eu?" comments. The disclosure-policy note above is the script.
- Watch for "what about LLM-as-judge in non-rubric domains?" pushback. The article's domain-applicability caveat is implicit; if commenters challenge generality, the response is the explicit version (finder/grader split works where the rubric is expressible; collapses where the rubric is the hard problem).
- Watch for whether the closing CTA earns specific shipped-system replies (audience filter worked) or generic "interesting!" reactions (filter too loose).
- If the post lands well as standalone evidence-from-own-work, the pattern (interleave a shipped-system post between arc posts) is worth repeating between V&V arc post 2 and the closer.
