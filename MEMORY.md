...
- **Audience correction worth carrying (2026-08-26), REFINED by measurement 2026-08-29.** The 2026-08-26 read was that the LinkedIn following is **old DSP colleagues and former students**, not the AI/V&V audience the `/writing/` corpus is written for, and a draft aimed at the V&V following had to be scrapped. That was the right thing to *write for*. It was the wrong thing to predict distribution from: the launch post reached 65,264 members with **Greater Bengaluru the top location, ahead of Amsterdam**, 36% Entry / 28% Senior, and Higher Education + Research Services at 9% each. The post left the following entirely and travelled on topic. **Carry both halves**: write for the audience you know, do not forecast reach from it. `docs/writing-guide.md` Section 2 describes the *article* reader; it describes neither the feed audience nor who actually shows up.
- **DSP launch reception measured 2026-08-29** (2 days old, analytics export; full block in `memory/external-comments.md`). **90,201 impressions, 65,264 reached, 516 saves, 472 reactions, 29 reposts, 17 comments, 729 click-throughs, 400 followers gained.** 10.6x the previous outlier and 54-144x the V&V cross-post cluster. **The finding that should change decisions: saves exceeded reactions**, against 0-1 saves on every prior post. A durable artifact distributes by being *kept*; an argument distributes by being *agreed with*, and the V&V arc has been optimising the second signal on an audience that produced it once. Comments cleared the hypothesis threshold in absolute terms (17 vs a forecast of 8) and failed it per impression at 1.9 per 10k, the thinnest yield in the log -- reach was won by the artifact, not the prompt. The `memory/hypothesis-log.md` [2026-08-27] bet is **half-resolved**: thresholds cleared, but its reasoning (former students drawn out) is unsupported by the demographics and needs the comment authors classified by hand, which the export cannot do.
- **Institutional attribution now differs by repo, deliberately (owner decision 2026-08-29).** `dsp-workshop` names no institution anywhere reader-facing -- five references removed and `tests/verify_state.sh check_no_affiliation` enforces it -- so the affiliation stays negotiable rather than given away after a 65k-reach launch. Rationale in `dsp-workshop/memory/feedback_institutional_attribution.md`. **This site is the opposite and stays that way**: the homepage hero credential and bio name the employer as career history, which is load-bearing credibility for the reader `docs/writing-guide.md` Section 2 describes. **The inconsistency is intentional -- withholding a product credit is not the same act as deleting a CV.** Do not reconcile them.
...
- **vmodel.eu article in progress, blocked on evidence (2026-08-29).** Outline at `drafts/few-findings-is-not-a-good-sign.md`, verification record at `docs/verification/few-findings-is-not-a-good-sign.md`. Thesis: an LLM reviewer's finding count is not a quality signal, because a thin document gives the model less to criticise and produces the same output as a good one. Supporting numbers **verified against primary** (`vmodel.eu/calibration/results/incose_calibration.json`, n=186 confirmed): subjective-term detector 18/11/6/0/0% across scores 1-5, monotone; EARS conforming share 7.6% -> 26.9%; post-tuning findings **0.67 per report**, not the "~1" the intermediate claims. One claim **must not be used as written** -- "vague terms 52% vs 22%" omits score 5 rising back to 40%, which is selective-endpoint reporting in an article arguing against exactly that. **The spine is untraced**: the +/-1 adjustment failure lives in `held_out_pipeline.json`, which exists only on gpu-server, outside version control, from a 3-hour batch job. Get it into the repo whether or not the article ships.
